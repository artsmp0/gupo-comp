...
export function useDataSource() {
  ...
  const props = instance.props as GupoTableProps;
  let pagination = props.pagination as TableProps['pagination'];
  const _dataSource = ref<Recordable[]>([]);
  const getRowSpanKey = (): string[] => {
    return (
      props.columns?.filter(col => col.needRowSpan).map(col => col.dataIndex as string) ||
      ([] as string[])
    );
  };
    ...
    if (props.sortConfig) {
      ...
      if (Array.isArray(sorter)) {
        ...
        sorterParams = sorter
          .sort(
            (a, b) =>
              (a!.column!.sorter as Recordable).multiple -
              (b!.column!.sorter as Recordable).multiple
          )
          .reduce((res, cur) => {
            return cur.order
              ? {
        ...
        sorterParams = sorter?.order && {
          [sortField]: sorter.field,
          [orderField]: order[sorter.order],
        };
      }
    }
    /**
  ...
  const filterListWithOldParams = <T = any>(params?: T) => {
    filterParams.value = params
      ? {
          ...filterParams.value,
          ...params,
  ...
  const refresh = (keepPage = true) => {
    if (keepPage) filterList(undefined, false);
    else filterList();
  };
