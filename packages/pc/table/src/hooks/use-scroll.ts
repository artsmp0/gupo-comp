...
export const useScroll = (
  ...
  dataSource: WritableComputedRef<Recordable<any>[]>
) => {
  const { props } = getCurrentInstance()!;
  const { scroll, isWithParentHeight } = toRefs(props as GupoTableProps);
  ...
  const calcDynamicHeight = async () => {
    ...
    if (antPaginationStyle) {
      marginBottom = antPaginationStyle.marginBottom
        ? parseInt(antPaginationStyle.marginBottom, 10)
        : 0;
