...
const defaultCheckedList = data?.columns.value
  .filter((col: any) => !col.notShow)
  .map((col: any) => col.dataIndex as string);
const defaultState = {
...
watch(
  ...
  (v: string[]) => {
    ...
    data.columns.value = sortColumns.value.filter((col: any) =>
      v.includes(col.dataIndex as string)
    );
  },
  { immediate: true }
);
...
const onCheckAllChange = (e: CheckboxChangeEvent) => {
  state.value.checkedList = e.target?.checked
    ? originColumns.value?.map((opt: any) => opt.dataIndex as string) || []
    : [];
};
...
async function handleVisibleChange() {
  ...
  sortIns = sortablejs.create(el, {
    ...
    onEnd: evt => {
      ...
      // 排序结束记得更新原始列的顺序
      data.columns.value = columns.filter((c: any) => data.columns.value.includes(c));
      ...
      <CheckboxGroup ref="$sortableWrapper" v-model:value="state.checkedList">
        ...
        <div v-for="item in originColumns" :key="item.dataIndex" class="column-setting__item">
          ...
          <Checkbox
            ...
            :disabled="
              data?.columns.value.length === 1 && data.columns.value[0].dataIndex === item.dataIndex
            "
          >
