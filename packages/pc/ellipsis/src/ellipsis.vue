...
<template>
  <Tooltip v-if="isOverflow && props.showTooltip" :placement="props.placement" :color="props.color">
    ...
    <span ref="$text" :title="showTitle" :class="ns.b()" :style="style" @click="toggleExpand">
      {{ props.text }}
    </span>
  </Tooltip>
  <span v-else ref="$text" :title="showTitle" :class="ns.b()" :style="style">
    <span
      v-if="props.expandTrigger"
      :title="isBlock ? '收起' : '展开'"
      :class="ns.e('toggle-btn')"
      @click="toggleExpand"
    >
